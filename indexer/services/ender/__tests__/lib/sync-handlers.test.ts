...
describe('syncHandler', () => {
  describe('addHandler/process', () => {
    it('successfully adds handler', async () => {
      ...
      syncHandlers.addHandler(FuryaIndexerSubtypes.MARKET, firstHandler);
      syncHandlers.addHandler(FuryaIndexerSubtypes.ASSET, secondHandler);
      // should be ignored, because transfers are not handled by syncHandlers
      syncHandlers.addHandler(FuryaIndexerSubtypes.TRANSFER, handlerNotInvoked);
