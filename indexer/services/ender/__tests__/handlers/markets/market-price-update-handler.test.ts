...
import { FuryaIndexerSubtypes, MarketPriceUpdateEventMessage } from '../../../src/lib/types';
import {
  defaultHeight,
...
import { MarketEventV1, IndexerTendermintBlock, IndexerTendermintEvent } from '@furyaprotocol-indexer/v4-protos';
...
describe('marketPriceUpdateHandler', () => {
  ...
  it('fails when no market exists', async () => {
    ...
    expect(loggerCrit).toHaveBeenCalledWith(expect.objectContaining({
      at: expect.stringContaining('PL/pgSQL function furya_market_price_update_handler('),
