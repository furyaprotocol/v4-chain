...
import { IndexerTendermintBlock, IndexerTendermintEvent, MarketEventV1 } from '@furyaprotocol-indexer/v4-protos';
import { KafkaMessage } from 'kafkajs';
import { onMessage } from '../../../src/lib/on-message';
import { FuryaIndexerSubtypes, MarketCreateEventMessage } from '../../../src/lib/types';
import {
  defaultHeight,
  defaultMarketCreate,
...
describe('marketCreateHandler', () => {
  ...
  it('errors when attempting to create an existing market', async () => {
    ...
    expect(loggerCrit).toHaveBeenCalledWith(expect.objectContaining({
      at: expect.stringContaining('PL/pgSQL function furya_market_create_handler('),
