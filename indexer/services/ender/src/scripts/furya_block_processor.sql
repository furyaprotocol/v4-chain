CREATE OR REPLACE FUNCTION furya_block_processor(block jsonb) RETURNS jsonb AS $$
/**
...
DECLARE
    ...
    rval_to_merge jsonb[];
BEGIN
    PERFORM furya_create_initial_rows_for_tendermint_block(block_height, block_time, block->'txHashes', block->'events');
    ...
    IF NOT block_height = 0 THEN
        rval = furya_block_processor_unordered_handlers(block);
        rval_to_merge = furya_block_processor_ordered_handlers(block);
    ELSE
        rval = furya_block_processor_ordered_handlers(block);
        rval_to_merge = furya_block_processor_unordered_handlers(block);
    END IF;
