CREATE OR REPLACE FUNCTION furya_funding_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - perpetual_markets: A mapping from perpetual market id to the associated perpetual market in perpetual-market-model format (https://github.com/furyanprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-market-model.ts).
...
BEGIN
    FOR funding_update IN SELECT * FROM jsonb_array_elements(event_data->'updates') LOOP
        ...
        perpetual_markets_response = jsonb_set(perpetual_markets_response, ARRAY[(perpetual_market_record."id")::text], furya_to_jsonb(perpetual_market_record));
        ...
        CASE event_data->'type'
            ...
            WHEN TYPE_FUNDING_RATE_AND_INDEX THEN
                ...
                event_id = furya_event_id_from_parts(block_height, transaction_index, event_index);

                funding_index_updates_record."id" = furya_uuid_from_funding_index_update_parts(
                    block_height,
                    event_id,
                ...
                funding_index_updates_record."rate" = furya_trim_scale(
                ...
                funding_index_updates_record."fundingIndex" = furya_trim_scale(
                    furya_from_serializable_int(funding_update->'fundingIndex') *
                    power(10,
                ...
                funding_update_response = jsonb_set(funding_update_response, ARRAY[(funding_index_updates_record."perpetualId")::text], furya_to_jsonb(funding_index_updates_record));
