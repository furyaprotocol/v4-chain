CREATE OR REPLACE FUNCTION furya_deleveraging_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - liquidated_fill: The created liquidated fill in fill-model format (https://github.com/furyanprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/fill-model.ts).
    - offsetting_fill: The created offsetting fill in fill-model format (https://github.com/furyanprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/fill-model.ts).
    - perpetual_market: The perpetual market for the deleveraging in perpetual-market-model format (https://github.com/furyanprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-market-model.ts).
    - liquidated_perpetual_position: The updated liquidated perpetual position in perpetual-position-model format (https://github.com/furyanprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
    - offsetting_perpetual_position: The updated offsetting perpetual position in perpetual-position-model format (https://github.com/furyanprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
...
BEGIN
    ...
    */
    size = furya_trim_scale(furya_from_jsonlib_long(event_data->'fillAmount') *
                                 power(10, perpetual_market_record."atomicResolution")::numeric);
    price = furya_trim_scale(furya_from_jsonlib_long(event_data->'price') *
                                  power(10, QUOTE_CURRENCY_ATOMIC_RESOLUTION)::numeric);

    liquidated_subaccount_uuid = furya_uuid_from_subaccount_id(event_data->'liquidated');
    offsetting_subaccount_uuid = furya_uuid_from_subaccount_id(event_data->'offsetting');
    ...
    VALUES (furya_uuid_from_fill_event_parts(event_id, 'TAKER'),
    ...
    INSERT INTO fills
        ("id", "subaccountId", "side", "liquidity", "type", "clobPairId", "size", "price", "quoteAmount",
         "eventId", "transactionHash", "createdAt", "createdAtHeight", "fee")
    VALUES (furya_uuid_from_fill_event_parts(event_id, 'MAKER'),
    ...
    liquidated_perpetual_position_record = furya_update_perpetual_position_aggregate_fields(
        liquidated_subaccount_uuid,
        perpetual_id,
        liquidated_side,
        size,
        price);
    offsetting_perpetual_position_record = furya_update_perpetual_position_aggregate_fields(
        offsetting_subaccount_uuid,
        perpetual_id,
        offsetting_side,
        size,
    ...
    RETURN jsonb_build_object(
            'liquidated_fill',
            furya_to_jsonb(liquidated_fill_record),
            'offsetting_fill',
            furya_to_jsonb(offsetting_fill_record),
            'perpetual_market',
            furya_to_jsonb(perpetual_market_record),
            'liquidated_perpetual_position',
            furya_to_jsonb(liquidated_perpetual_position_record),
            'offsetting_perpetual_position',
            furya_to_jsonb(offsetting_perpetual_position_record)
        );
END;
$$ LANGUAGE plpgsql;
