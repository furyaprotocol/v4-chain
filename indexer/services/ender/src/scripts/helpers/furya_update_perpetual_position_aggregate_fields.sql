CREATE OR REPLACE FUNCTION furya_update_perpetual_position_aggregate_fields(
...
BEGIN
    ...
    IF furya_perpetual_position_and_order_side_matching(perpetual_position_record."side", side) THEN
        sum_open := furya_trim_scale(perpetual_position_record."sumOpen" + size);
    ...
    ELSE
        sum_close := furya_trim_scale(perpetual_position_record."sumClose" + size);
