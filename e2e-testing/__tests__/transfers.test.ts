...
import {
  ...
  SubaccountInfo,
  ValidatorClient,
} from '@furyaprotocol/v4-client-js';
import {
  Ordering,
  SubaccountTable,
  TransferColumns,
...
import { FURYA_LOCAL_ADDRESS, FURYA_LOCAL_MNEMONIC } from './helpers/constants';

describe('transfers', () => {
  it('test deposit', async () => {
    connectAndValidateSocketClient();
    const wallet = await LocalWallet.fromMnemonic(FURYA_LOCAL_MNEMONIC, BECH32_PREFIX);
    ...
    const response = await indexerClient.account.getSubaccountTransfers(FURYA_LOCAL_ADDRESS, 0);
    ...
    assetPosResp = await indexerClient.account.getSubaccountAssetPositions(FURYA_LOCAL_ADDRESS, 0);
    expect(assetPosResp).not.toBeNull();
      ...
      (message) => {
        if (typeof message.data === 'string') {
          ...
          if (data.type === 'connected') {
            mySocket.subscribeToSubaccount(FURYA_LOCAL_ADDRESS, 0);
          } else if (data.type === 'subscribed') {
            expect(data.channel).toEqual('v4_subaccounts');
            expect(data.id).toEqual(`${FURYA_LOCAL_ADDRESS}/0`);
            expect(data.contents.subaccount).toEqual(
              expect.objectContaining({
                address: FURYA_LOCAL_ADDRESS,
                subaccountNumber: 0,
              }),
            );
