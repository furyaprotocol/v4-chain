...
import (
	...
	pricefeed "github.com/furyanprotocol/v4-chain/protocol/daemons/pricefeed/client/types"
	"github.com/furyanprotocol/v4-chain/protocol/testutil/constants"
	pricefeed_testutil "github.com/furyanprotocol/v4-chain/protocol/testutil/pricefeed"
...
func (t *Testnet) initializeNode(moniker string) (*Node, error) {
	...
	if t.isPreupgradeGenesis {
		entrypointCommand = "/furyaprotocol/preupgrade_entrypoint.sh"
	} else {
		entrypointCommand = "furyaprotocold"
	}

	resource, err := t.pool.RunWithOptions(
		&dockertest.RunOptions{
			Name:       fmt.Sprintf("testnet-local-%s", moniker),
			Repository: "furyaprotocol-container-test",
			Tag:        "",
			...
			Entrypoint: []string{
				...
				"start",
				"--home",
				fmt.Sprintf("/furyaprotocol/chain/.%s", moniker),
			...
			Env: []string{
				"DAEMON_NAME=furyaprotocold",
				fmt.Sprintf("DAEMON_HOME=/furyaprotocol/chain/.%s", moniker),
