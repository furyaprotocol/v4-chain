...
download_preupgrade_binary() {
	...
	tar_url="https://github.com/furyanprotocol/v4-chain/releases/download/protocol%2F$PREUPGRADE_VERSION/furyaprotocold-$PREUPGRADE_VERSION-linux-$url_arch.tar.gz"
	tar_path='/tmp/furyaprotocold/furyaprotocold.tar.gz'
	mkdir -p /tmp/furyaprotocold
	curl -vL $tar_url -o $tar_path
	furyaprotocold_path=$(tar -xvf $tar_path --directory /tmp/furyaprotocold)
	cp /tmp/furyaprotocold/$furyaprotocold_path /bin/furyaprotocold_preupgrade
}
