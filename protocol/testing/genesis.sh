...
SUBACCOUNTS_MODACC_ADDR="furya1v88c3xv9xyv3eetdx0tvcmq7ung3dywp5upwc6"
REWARDS_VESTER_ACCOUNT_ADDR="furya1ltyc6y4skclzafvpznpt2qjwmfwgsndp458rmp"
# Address of the `bridge` module account.
# Obtained from `authtypes.NewModuleAddress(bridgetypes.ModuleName)`.
BRIDGE_MODACC_ADDR="furya1zlefkpe3g0vvm9a4h0jf9000lmqutlh9jwjnsv"
...
function edit_genesis() {
	...
	# Market: FURYA-USD
	dasel put -t json -f "$GENESIS" '.app_state.prices.market_params.[]' -v "{}"
	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[34].pair' -v 'FURYA-USD'
	...
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[34].price' -v '2050000000'          # $2.05 = 1 FURYA.
	# FURYA Exchange Config
	furya_exchange_config_json=$(cat "$EXCHANGE_CONFIG_JSON_DIR/furya_exchange_config.json" | jq -c '.')
	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[34].exchange_config_json' -v "$furya_exchange_config_json"
