...
import (
	...
	"github.com/furyanprotocol/v4-chain/protocol/x/rewards"
	rewards_keeper "github.com/furyanprotocol/v4-chain/protocol/x/rewards/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/furyaprotocol/v4/rewards/params", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/furyaprotocol/v4/rewards/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
