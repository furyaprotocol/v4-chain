...
# This script takes in the result of a `/furyaprotocol/clob/clob_pair` query, and generates 
# a sample governance proposal to enable trading on all CLOB pairs. 
# Example usage:
# 1. Get all clob pairs from a REST endpoint:
#   % curl -X GET "https://furya-testnet-archive.allthatnode.com:1317/furyaprotocol/clob/clob_pair" -H "accept: application/json" > /tmp/clob_pairs.json
...
# 3. Submit proposal:
#   % furyaprotocold tx gov submit-proposal /tmp/proposal_enable_trading_all_markets.json --from alice --gas auto --fees 400000000000000000adv4tnt

# Constants
NINE_ZEROS="000000000"
AUTHORITY="furya10d07y265gmmuvt4z0w9aw880jnsr700jnmapky"
...
MESSAGES=$(jq --arg authority "$AUTHORITY" '
  .clob_pair | map({
    "@type": "/furyaprotocol.clob.MsgUpdateClobPair",
    "authority": $authority,
    "clob_pair": {
      "id": .id,
